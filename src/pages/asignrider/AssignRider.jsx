...
const AssignRider = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ parcelId, riderId }) => {
      // Update parcel status to 'transit' and assign rider
    ...
    onSuccess: () => {
      ...
      setModalOpen(false);
      MySwal.fire('Success', 'Rider assigned and statuses updated!', 'success');
    },
    onError: () => {
      MySwal.fire('Error', 'Failed to assign rider or update status.', 'error');
    }
  });
                    ...
                    {riders.map((rider) => {
                      ...
                      return (
                        ...
                        className={`card border hover:shadow-lg transition ${
                          matches ? 'border-green-500 bg-green-800 text-white' : 'border-base-600'
                        }`}
                      >
